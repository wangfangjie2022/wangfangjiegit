...
#if 1	
  // ARM Compiler V5
  __IO uint32_t gGotoLoaderFlag   __attribute__((at(RAM_BASE_ADDR)));
  __IO uint32_t gLoderResetFlag   __attribute__((at(RAM_BASE_ADDR+4)))=0;
//__IO uint32_t gBatPercent       __attribute__((at(RAM_BASE_ADDR+8)))=0;
#else
  // ARM Compiler V6
  __IO uint32_t gGotoLoaderFlag   __attribute__((section(".ARM.__at_0x24000000")));
  __IO uint32_t gLoderResetFlag   __attribute__((section(".ARM.__at_0x24000004")))=0;
#endif
...
uint32_t flash_addr= FLASH_CFGINFO_ADDR1;
uint32_t write_len= 10;
uint8_t  test_buf[256];
uint8_t  write_en= 0;
uint8_t  erase_en= 0;
uint8_t  bytewrite_en= 0;
uint8_t  bytewrite_len=0;
uint32_t bytewrite_cnt=0;
uint32_t bytewrite_offset=0;
...
void flash_test(void)
{
  ...
  if(write_en)
  {
    ...
    FlashWrite_AppWords(flash_addr, test_buf, write_len);
    write_en= 0;
  ...
  if(bytewrite_en)
  {
    ...
    for(uint32_t i=0; i<bytewrite_cnt; i++)
    {
      FlashWrite_AppWords(flash_addr+bytewrite_offset, test_buf, bytewrite_len);
      bytewrite_offset += bytewrite_len;
    }
    FlashWrite_ChkAppRemainWords();
    bytewrite_en= 0;
  }
  ...
  if(erase_en)
  {
    ...
    FirstSector= GetSector(flash_addr);
    FlashErase_NSectors(FirstSector, NbOfSectors, Banks);
    erase_en= 0;
...
{
	/* IWDG 4s 超时溢出 */ 
  //MX_IWDG_Init(IWDG_PRESCALER_128,1250);
  /* 启动独立看门狗 */
  ...
  boot_info_init();
  if(gGotoLoaderFlag == (RC_UPDATE_FLAG|0x2))
  {
    RF_Init();
    gTogglePowerSate();
  }
	else if(g_boot_cfg.magic == MAGIC_VALUE)
  {
    gGotoLoaderFlag = 0;
    if(CheckAppCode() > 0)
    {
      disable_all_irq();
      do_jump(FLASH_APPLICATION_ADDR);
    }
    RF_Init();
	}
  ...
  while (1)
  {
		...
		if (ENUM_POWER_ON == gGetCurPowerState()) {
			BuzzOn_time(1,0,1,50, 950 );
    }
    //flash_test();

    if(gLoderResetFlag == (RC_UPDATE_FLAG|0xFF) )
    {	
